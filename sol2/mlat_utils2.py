...
def calc_mlat(sensor_ids, sensor_locations, sensor_timestamps, time_deltas, debug=False):
    ...
    calculated_locations = list()
    for s1, s2, s3, s4 in itertools.combinations(sorted(sensor_ids), 4):
        relevant_sensors = [s1, s2, s3, s4]
        possible = True
        for i, j in itertools.combinations(relevant_sensors, 2):
            if i not in time_deltas or j not in time_deltas[i]:
                possible = False
                break
        if not possible:
            continue
        ...
        for sensor_id in relevant_sensors[1:]:
            timestamps.append(
                sensor_timestamps[sensor_id] + time_deltas[s1][sensor_id][0]
                - sensor_timestamps[s1]
            )
    ...
    if not len(calculated_locations):
        return

    # return mean calculated location
    #sum_loc = [0, 0, 0]
